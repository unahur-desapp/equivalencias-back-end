...
describe('Endpoints GET de Usuarios', () => {
  test('Debe retornar una lista de todos los usuarios', async () => {
    ...
    expect(response.body[0].apellido).toBe('Fernandez');
    expect(response.body[0].nombre).toBe('Enzo');

    expect(response.body[2].apellido).toBe('Rey');
    expect(response.body[2].nombre).toBe('Carlos');
  });
  ...
  test('Debe retornar un array de todos los usuarios', async () => {
    ...
    //console.log(response)
    expect(response.status).toBe(200);
    expect(response.body.data[0].apellido).toBe('Fernandez');
    expect(response.body.data[0].nombre).toBe('Enzo');
    expect(response.body.data[1].apellido).toBe('kiedis');
    expect(response.body.data[1].nombre).toBe('Anthony ');
  });
