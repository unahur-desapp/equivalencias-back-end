...
export const createEquivalencia = async (req, res) => {
  ...
  //const datosUniversidadOrigen = pick(EquivalenciaData, ["id", "codigo", "nombre","localidad","sigla"]);

  const EquivalenciaConTablas = {
    ...datosEquivalencia,
    Materias_aprobadas: [datosMateriaAprobada],
    //Universidad_origne:[datosUniversidadOrigen],
...
/*
export const createEquivalencia = async (req, res) => {
  const materiaAprobada = req.body;
  const equivalencia = await Equivalencia.findByPk(materiaAprobada.EquivalenciumId);

  if (!equivalencia) {
    return res.status(400).send({
      error: `No existe la equivalencia con el id ${materiaAprobada.EquivalenciumId}, por favor ingresá otra.`,
    });
  }

  const data = await Materia_aprobada.create(materiaAprobada);
