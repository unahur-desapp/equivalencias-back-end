...
export const getEquivalenciasGeneral = async (req, res) => {
  // Aca abria que incluir la entidad Carreras.
  const equival = await Equivalencia.findAll({
...
export const getEquivalenciasGeneralID = async (req, res) => {
  ...
  if (equi) {
    ...
    res
      .status(404)
      .json({ message: `Equivalencia ${equivalenciaid} no encontrada` });
  }
};
...
export const createEquivalenciax3 = async (req, res) => {
  ...
  const datosMateriaSolicitada = pick(EquivalenciaData, ['id', 'nombre']);
  ...
  const datosCompletarMateriaSolicitada = {
    ...datosMateriaSolicitada,
    EquivalenciumId: dbResponse.id,
  };

  Materia_solicitada.create(datosCompletarMateriaSolicitada);
};
