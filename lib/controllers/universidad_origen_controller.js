...
export const createUniversidadOrigen = async (req, res) => {
  const { nombre_universidad, localidad, sigla } = req.body;

  if (!nombre_universidad || !localidad || !sigla) {
    return res.status(400).json({ error: 'Faltan campos obligatorios' });
  }

  try {
    const universidadExistente = await Universidad_origen.findOne({
      where: { nombre_universidad },
    });

    if (universidadExistente) {
      return res.status(409).json({ error: 'La universidad ya existe' });
    }

    const nuevaUniversidad = await Universidad_origen.create({
      nombre_universidad,
      localidad,
      sigla,
      disabled: false,
