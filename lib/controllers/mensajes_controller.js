...
import Equivalencia from '../models/equivalencia.js';

export const getChat = async (req, res) => {
  // Obtener el ID de equivalencia del chat desde los parámetros de la solicitud
  const equivalenciaId = req.params.id;
  // Consulta mensajes del chat: trae resgistros de mensajes, referentes al id pasado
  // por parametro, incluyendo las relaciones entre mensajes-equivalencias y equivalencia-usuarios
  const mensajes = await Mensajes.findAll({
    include: [
      {
        model: Equivalencia,
        attributes: ['id', 'estado', 'UsuarioId', 'carrera'],
        include: [
          {
            model: Usuario,
            as: 'Usuario',
...
export const getChatVieja = async (req, res) => {
  try {
    // Obtener el ID de equivalencia del chat desde los parámetros de la solicitud
    const equivalenciaId = req.params.id;
